...
function updatePeakCursor() {
    d3.select('.peak-cursor')
        .style('display', hoveredActivity ? 'inline' : 'none');

    if(!hoveredActivity) return;

    var x = xScale(hoveredActivity.peakMinutes) + labelWidth;
    var y = yScale(1) + hoveredActivity.yOffset;
    var t = peakCursorText(hoveredActivity.peakMinutes);

    d3.select('.peak-cursor')
        .attr('transform', 'translate(' + x + ',' + y + ')');

    d3.select('.peak-cursor text')
        .text(t);
}
